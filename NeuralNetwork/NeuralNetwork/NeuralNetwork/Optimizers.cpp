...
void SGD::update(MatrixXd& weights_)
{
	weights_ -= learningRate * gradient;
}
...
void SGDMomentum::update(MatrixXd& weights_)
{
	weightsVelocity = gamma * weightsVelocity + learningRate * gradient;
...
void RMSProp::update(MatrixXd& weights_)
{
	MatrixXd gradient2 = gradient.unaryExpr([&](double v) {return v * v; });
	st = rho * st + (1 - rho) * gradient2;
	MatrixXd radicalSt = st.unaryExpr([&](double v) {return learningRate / std::sqrt(v + epsilon); });
	weights_  -= gradient.cwiseProduct(radicalSt);
}
...
void Adam::update(MatrixXd& weights_)
{
	firstMomentum = beta1 * firstMomentum + (1 - beta1) * gradient;
	MatrixXd gradient2 = gradient.unaryExpr([&](double v) {return v * v; });
...
void AdaGrad::update(MatrixXd& weights_)
{
	squareGradient += gradient.unaryExpr([&](double v) {return v * v; });
	MatrixXd invGradient = gradient.unaryExpr([&](double v) {return 1.0 / (std::sqrt(v) + epsilon); });
	weights_ -= learningRate * invGradient.cwiseProduct(gradient);
}
