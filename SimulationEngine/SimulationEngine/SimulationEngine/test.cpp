...
TEST_CASE("Testing removing a particle from the particle list") {
    ...
    auto particles = std::make_unique<Particles>(nmax);
    // adding each particle to the list
    ...
    for (int i = 0; i < nXs; i++) {
        ...
        double newR = r[i];
        double newM = m[i];
        // adding this particle
        particles->addParticle(newX, newV, newF, newM, newR);
    }
