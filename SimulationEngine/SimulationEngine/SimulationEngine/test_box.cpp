...
TEST_CASE("Testing another box object") {
	std::cout << "Testing another box object" << std::endl;
	std::cout << std::string(80, '=') << std::endl;

	// Engine
	Engine engine;
	// box dimensions
	array<double, 3> min = { -350.0,-200.0,-150.0 };
	array<double, 3> max = { 35.0,75.0,900.0 };
	// box 
	auto box = make_unique<Box>(min, max);
	// injecting the dependencies
	box->injectDependencies(engine);
