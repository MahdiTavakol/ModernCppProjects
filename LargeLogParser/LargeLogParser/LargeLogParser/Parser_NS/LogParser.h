...
namespace Parser_NS {
	...
	public:
		...
		LogParser(const LogParser& rhs_) = delete;
		LogParser(LogParser&& rhs_) noexcept = default;
		LogParser& operator=(const LogParser& rhs_) = delete;
		LogParser& operator=(LogParser&& rhs_) noexcept = default;
		virtual ~LogParser() {
			if (file.is_open())
				file.close();
		}
