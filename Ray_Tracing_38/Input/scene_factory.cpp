...
void scene_factory::setup_cornell_smoke()
{
	...
	world->add(std::make_unique<constant_medium>(std::move(box1), 0.1, color(0, 0, 0), *list));
	world->add(std::make_unique<constant_medium>(std::move(box2), 0.1, color(1, 1, 1), *list));

	auto bvh = std::make_unique<bvh_node>(std::move(world), bvh_mode);
	world = std::make_unique<hittable_list>(std::move(bvh));

	
	//std::unique_ptr<hittable> smoked_world = std::make_unique<constant_medium>(std::move(world), 0.01, color(0, 0, 0), *list);

	//world = std::make_unique<hittable_list>();
	//world->add(std::move(smoked_world));
