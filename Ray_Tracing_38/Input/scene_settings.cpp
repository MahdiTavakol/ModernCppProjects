...
void scene_settings::return_scene_map(std::map<std::string, int>& scene_map_)
{
	scene_map_ =
	{
		...
		{"FINAL_SCENE_DETAILED", FINAL_SCENE_DETAILED},
...
void scene_settings::extra_parse()
{
	...
	else if (bvh_mode_str == "MIDDLE")
		bvh_mode = BVH_Split_Method::MIDDLE;
	else if (bvh_mode_str == "SAH_SIMPLE")
		bvh_mode = BVH_Split_Method::SAH_SIMPLE;
	else
	{
		std::string error_text = "Unknown BVH split method: " + bvh_mode_str;
		throw std::invalid_argument(error_text);
	}
