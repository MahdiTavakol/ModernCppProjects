...
public:
    double e[4];
    vec4() : e{ 0, 0, 0, 0 } {}
    vec4(double _e0, double _e1, double _e2, double _e3) :
        e{ _e0, _e1, _e2, _e3 } {}
    vec4(const vec4& rhs_) :
        e{ rhs_[0],rhs_[1],rhs_[2], rhs_[3]} {
    }
    vec4& operator=(const vec3& rhs_) {
        e[0] = rhs_[0]; e[1] = rhs_[1]; e[2] = rhs_[2]; e[3] = rhs_[3];
        return *this;
    }
...
public:
    float e[2];
    vec2() : e{ 0, 0 } {}
    vec2(float _e0, float _e1) : e{ _e0, _e1 } {}
    vec2(const vec2& rhs_) :
        e{ rhs_[0],rhs_[1] } {
    }
    vec2& operator=(const vec2& rhs_) {
...
inline bool equal(const vec3& a_, const vec3& b_, float tol_) noexcept
{
    return (
        std::abs(a_.x() - b_.x()) <= tol_ &&
        std::abs(a_.y() - b_.y()) <= tol_ &&
