...
std::chrono::year_month_day ChronoDate::ymd() const { return date_; }

int ChronoDate::year() const { return static_cast<int>(date_.year()); }

unsigned ChronoDate::month() const {
  return static_cast<unsigned>(date_.month());
}

unsigned ChronoDate::day() const { return static_cast<unsigned>(date_.day()); }
...
bool ChronoDate::is_leap_year() const { return date_.year().is_leap(); }
...
std::istream &operator>>(std::istream &is, ChronoDate &rhs) {
  ...
  if (std::getline(ss, year_str, '-') && std::getline(ss, month_str, '-') &&
    ...
    throw std::invalid_argument(
        "ChronoDate input format error. Expected format: YYYY-MM-DD");
  }
