...
TaskManager::TaskManager(const std::string &filename, SortKey _sort_key)
    : sort_key{_sort_key} {
  Initialize();
  loadFromFile(filename);
  sortTasks();
...
TaskManager::~TaskManager() { saveToFile("Tasks-desructor.txt"); }

void TaskManager::Initialize() { Tasks.clear(); }

void TaskManager::sortTasks() {
  SortKey sort_key_ = this->sort_key;
  auto sortLambda = [&sort_key_](const std::unique_ptr<Task> &a,
                                 const std::unique_ptr<Task> &b) -> bool {
...
void TaskManager::addTask(std::string title, int priority,
                          const std::string &due_date_str) {
  ChronoDate due_date;
  std::stringstream ss(due_date_str);
  ss >> due_date;
  std::unique_ptr<Task> newTask =
      std::make_unique<Task>(std::move(title), priority, std::move(due_date));
  Tasks.push_back(std::move(newTask));
  sortTasks();
}

void TaskManager::finishTaskByTitle(const std::string &title) {
  auto ip = std::find_if(Tasks.begin(), Tasks.end(),
...
void TaskManager::listTasks(
    const std::vector<std::unique_ptr<Task>> &inputTasks,
    const std::string &description, const Category &cat) const {
  if (inputTasks.empty()) {
  ...
  std::array<std::string, 4> headers = {"Task Name", "Priority", "Due Date",
                                        "Progress"};
  ...
  auto print_line = [](const std::unique_ptr<Task> &task) {
    std::cout << std::format("{:<20}\t{:^10}\t{:^15}\t{:^8.1f}\n",
                             ...
                             task->getDueDate().ymd(), task->getProgress())
              << std::endl;
  };
  ...
  std::for_each(inputTasks.begin(), inputTasks.end(),
                [&cat, &print_line](const std::unique_ptr<Task> &task) {
...
TaskManager::filterTasks(const Category &cat) const {
  std::vector<std::unique_ptr<Task>> filteredTasks;
  std::for_each(Tasks.begin(), Tasks.end(),
                [&cat, &filteredTasks](const std::unique_ptr<Task> &task) {
                  if (task->getCategory() == cat) {
...
void TaskManager::saveToFile(const std::string &filename) const {
  ...
  for (const auto &task : Tasks) {
    ...
    ss >> due_date_str;
    file << std::format("{:<20} {:^10} {:^15} {:^8}\n", task->getName(),
                        task->getPriority(), due_date_str, task->getProgress());
  }
}
...
void TaskManager::loadFromFile(const std::string &filename) {
  ...
  while (std::getline(file, line)) {
    ...
    if (!(std::getline(ss, name, ',') && std::getline(ss, priority_str, ',') &&
          std::getline(ss, due_date_str, ',')))
      throw std::runtime_error(
          "Error reading the task number " + std::to_string(task_number) +
          "from the file : " + filename + ". Please check the file format");
    priority = std::stoi(priority_str);
    std::stringstream ss2(due_date_str);
    ss2 >> due_date;
    std::unique_ptr<Task> newTask =
        std::make_unique<Task>(std::move(name), priority, std::move(due_date));
    Tasks.push_back(std::move(newTask));
...
void TaskManager::printHighPriorityTasks(const int &threshold) const {
  ...
  highPriorityTasks.reserve(
      Tasks.size()); // Reserve space to avoid multiple allocations
  std::for_each(
      Tasks.begin(), Tasks.end(),
      [&threshold, &highPriorityTasks](const std::unique_ptr<Task> &task) {
        if (task->getPriority() >= threshold)
          highPriorityTasks.push_back(std::make_unique<Task>(*task));
      });
  ...
  auto conditional_append = [&threshold, &highPriorityTasksStructs](
                                const std::unique_ptr<Task> &task) {
    if (task->getPriority() >= threshold) {
      TaskStruct taskStruct{task->getName(), task->getPriority(),
                            task->getDueDate(), task->getProgress()};
  ...
  this->listTasks(highPriorityTasks, "High Priority Tasks (Priority >= " +
